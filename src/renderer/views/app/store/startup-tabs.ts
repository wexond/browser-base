...
export class StartupTabsStore {
  ...
  public async addStartupTabItem(item: IStartupTab){
    const itemToReplace = this.list.find(x => 
                                            x.id === item.id &&
                                            x.windowId === item.windowId);
    if (itemToReplace) {
      this.db.update(itemToReplace, item);
      this.list[this.list.indexOf(itemToReplace)] = {...itemToReplace, ...item};
    }
    else { 
  ...
  public async addStartupDefaultTabItems(items: IStartupTab[]){
    ...
    items.filter(x => x.url !== undefined && x.url.length > 1).forEach(async x => {
  ...
  public removeStartupTabItem(tabId: number, windowId: number){
    const itemToDelete = this.list.find(x => 
      x.id === tabId &&
      x.windowId === windowId);
      if (itemToDelete){
        this.list = this.list.filter(x => x.id !== tabId || x.windowId !== windowId);
