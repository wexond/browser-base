...
function getMpegtsPipeline(input, audioStream,videoStream, inputvideoCodec,subtitleStream,isDeinterlacingEnabled,handleConversionError,handleErroneousStreamError) {
    if((process.platform === 'darwin')){
        input += '?fifo_size=13160&overrun_nonfatal=1&buffer_size=/18800&pkt_size=188'
    }
    console.log('run url: ',input)
    let ffmpegCmd = initializeCmd(input)
...
exports.getMpegtsPipeline = getMpegtsPipeline
