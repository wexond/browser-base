...
readdirSync(join(__dirname, 'api')).forEach((file) => {
  if (!file.endsWith('.json')) return;
  const content = readFileSync(join(__dirname, `./api/${file}`), 'utf8');
  const json = JSON.parse(stripJsonComments(content))[0];
  apis[json.namespace] = json;
});
...
output += `var p=${JSON.stringify(parameters).replace(/"([^"]+)":/g, '$1:')};`;
...
output += `var f=${stripJsonComments(
  readFileSync(join(__dirname, '_api_features.json'), 'utf8'),
)};`;
